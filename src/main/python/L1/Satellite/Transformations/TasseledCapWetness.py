...
class TasseledCapWetness(L1_Input):
    def __init__(self,
        ...
        if use_openeo:
            ...
            logger.info(f"Downloading {self.transformation_name} using openEO for {shapefile} between {start_date} and {end_date}")
    ...
    def _download_workflow_tct(self, connection, filename, shape, start_date, end_date):
        ...
        bands = ["B01", "B02", "B03", "B04", "B05", "B06", "B07", "B08", "B8A", "B09", "B11", "B12"] #B10 not in L2A
        # Define datacube
        ...
        # Compute Tasseled Cap Transformation for Wetness
        # Sentinel-2 L2A data from openEO is stored as scaled integers (DN = reflectance * 10000).
        # TCT coefficients are designed for reflectance in [0, 1], so we must divide by 10000.
        WETNESS_COEFS = [0.1825, 0.1763, 0.1615, 0.0486,
                         0.0170, 0.0223, 0.0219, -0.0755,
                         -0.0910, -0.1369, -0.7701, -0.5293]

        wetness = reduce(
            add,
            (datacube.band(band) / 10000.0 * coef for band, coef in zip(bands, WETNESS_COEFS))
        )

        job = wetness.execute_batch(
            filename,
            title=f"Download field {self.transformation_name} data of {shape} for the period {start_date} to {end_date}",
    ...
    def _download_datacube(self, shapefile_name, start_date, end_date):
        ...
        filename = os.path.join(output_dir, f"s2_l2a_{self.transformation_name}_{shapefile_name}_{start_date}_{end_date}.nc")
        ...
        self._download_workflow_tct(connection, filename, shape, start_date, end_date)
        logger.info(f"Downloaded {self.transformation_name} datacube to {filename}")

        # Read the datacube
        ds = xr.open_dataset(filename)
        ds = ds.rename_vars({"var": self.transformation_name})
        return ds
    ...
    def _read_datacube(self, start_date, end_date, time_indices=None):
        ...
        # Sentinel-2 L2A data is stored as scaled integers (DN = reflectance * 10000).
        # TCT coefficients are designed for reflectance in [0, 1], so we must divide by 10000.
        WETNESS_COEFS = [0.1825, 0.1763, 0.1615, 0.0486,
                         0.0170, 0.0223, 0.0219, -0.0755,
                         -0.0910, -0.1369, -0.7701, -0.5293]
        ...
        ds = reduce(
            add,
            (datacube_subset[band] / 10000.0 * coef for band, coef in zip(bands, WETNESS_COEFS))
        ).to_dataset(name="TasseledCapWetness")

        print(f"Processed {self.transformation_name} for time index {resolved_indices}")
        return ds
    ...
    def get_debug_image(self):
        logger.info(f"Generating debug image for {self.transformation_name}")
        time_index = self.debug_time_index
        wetness_debug = self.datacube[self.transformation_name].isel(t=time_index).values

        # Configure the color palette
        norm = colors.Normalize(vmin=-1, vmax=1)
        rgba = cm.get_cmap("Blues")(norm(wetness_debug))
        rgb = (rgba[:, :, :3] * 255).astype(np.uint8)
        wetness_debug_img = Image.fromarray(rgb, mode="RGB")
        logger.info("Debug image generated successfully")

        return wetness_debug_img

    def get_rgb_image(self, time_index: int):
        logger.info(f"Fetching RGB image at time {time_index} for bands {self.transformation_name}")
    ...
    def get_sun_angles(self, time_index: int):
        raise NotImplementedError(f"get_sun_angles not implemented for {self.transformation_name}")

    def get_view_angles(self, time_index: int):
        raise NotImplementedError(f"get_view_angles not implemented for {self.transformation_name}")

    def get_dem(self, time_index: int):  # Digital Elevation Model (similar to depth map)
        raise NotImplementedError(f"get_dem not implemented for {self.transformation_name}")

    def get_cloud_mask(self, time_index: int) -> np.ndarray:
        raise NotImplementedError(f"get_cloud_mask not implemented for {self.transformation_name}")

    def get_ground_truth(self, time_index: int, band_indices: list[str]): # TODO NOTE GORUND TRUTH IS HARD CODED!!!
        raise NotImplementedError(f"get_ground_truth not implemented for {self.transformation_name}")
