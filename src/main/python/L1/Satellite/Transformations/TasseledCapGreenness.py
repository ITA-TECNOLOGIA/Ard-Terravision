...
class TasseledCapGreenness(L1_Input):
    def __init__(self,
    ...
    def _download_workflow_tct(self, connection, filename, shape, start_date, end_date):
        ...
        # Compute Tasseled Cap Transformation for Greenness

        # TCT coefficients are designed for reflectance in [0, 1], so we must divide by 10000.
        GREENNESS_COEFS = [-0.2266, -0.2818, -0.3020, -0.4283,
                           -0.2959, 0.1602, 0.3127, 0.3138,
                           0.4261, 0.1454, -0.1341, -0.2538]

        greenness = reduce(
            add,
            (datacube.band(band) / 10000.0 * coef for band, coef in zip(bands, GREENNESS_COEFS))
        )

        job = greenness.execute_batch(
            filename,
    ...
    def _read_datacube(self, start_date, end_date, time_indices=None):
        ...
        ds = reduce(
            add,
            (datacube_subset[band] / 10000.0 * coef for band, coef in zip(bands, GREENNESS_COEFS))
        ).to_dataset(name="TasseledCapGreenness")
    ...
    def get_debug_image(self):
        ...
        greenness_debug = self.datacube[self.transformation_name].isel(t=time_index).values

        # Configure the color palette
        norm = colors.Normalize(vmin=-1, vmax=1)
        rgba = cm.get_cmap("Greens")(norm(greenness_debug))
        rgb = (rgba[:, :, :3] * 255).astype(np.uint8)
        greenness_debug_img = Image.fromarray(rgb, mode="RGB")
        logger.info("Debug image generated successfully")

        return greenness_debug_img
