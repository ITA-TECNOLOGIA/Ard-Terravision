...
class EVI(L1_Input):
    ...
    def _download_workflow_spectral_index(self, connection, filename, shape, start_date, end_date):
        ...
        # Compute EVI
        evi = 2.5 * (datacube.band('B08') - datacube.band('B04')) / (datacube.band('B08') + 6*datacube.band('B04') - 7.5*datacube.band('B02') + 1)

        job = evi.execute_batch(
            filename,
    ...
    def _read_datacube(self, start_date, end_date, time_indices=None):
        ...
        ds = (2.5 * (datacube_subset['B08'] - datacube_subset['B04']) / (datacube_subset['B08'] + 6*datacube_subset['B04'] - 7.5*datacube_subset['B02'] + 1)).to_dataset(name="EVI")
    ...
    def get_debug_image(self):
        ...
        evi_debug = self.datacube[self.spectral_index].isel(t=time_index).values
