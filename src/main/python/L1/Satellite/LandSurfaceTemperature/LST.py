...
class LST(L1_Input):
    def __init__(self,
        ...
        if use_openeo:
            ...
            if datacube_path is None or not os.path.exists(datacube_path):
                raise FileNotFoundError(
                    f"LST datacube not found at '{datacube_path}'. "
                    f"LST cannot be computed from Sentinel-2 datacubes. "
                    f"It must be downloaded via OpenEO first (use use_openeo=True)."
                )
            logger.info(f"Reading LST datacube from: {datacube_path}")
    ...
    def _filter_by_valid_pixels(self, ds: xr.Dataset) -> xr.Dataset:
        if self.valid_pixel_threshold <= 0:
            return ds

        data = ds[self.spectral_index]
        spatial_dims = [d for d in data.dims if d != 't']
        valid_fraction = data.notnull().mean(dim=spatial_dims).values * 100

        valid_mask = valid_fraction >= self.valid_pixel_threshold
        valid_indices = np.where(valid_mask)[0]

        n_total = len(valid_fraction)
        n_dropped = n_total - len(valid_indices)

        if n_dropped > 0:
            dropped_indices = np.where(~valid_mask)[0]
            logger.info(f"Valid pixel threshold: {self.valid_pixel_threshold:.1f}%. "
                        f"Keeping {len(valid_indices)}/{n_total} timesteps, dropping {n_dropped}.")
            for idx in dropped_indices:
                logger.info(f"  Dropped timestep {idx}: {valid_fraction[idx]:.2f}% valid pixels")

        if len(valid_indices) == 0:
            logger.warning(f"All timesteps below valid pixel threshold ({self.valid_pixel_threshold}%). "
                           f"Returning data without filtering.")
            return ds

        return ds.isel(t=valid_indices)

    def _download_workflow_lst(self, connection, filename, shape, start_date, end_date):
        ...
        lst = datacube.band("LST")
        confidence = datacube.band("confidence_in")

        cloud_mask = confidence >= 16384
        lst_masked = lst.mask(cloud_mask)

        lst_celsius = lst_masked - 273.15

        job = lst_celsius.execute_batch(
            filename,
            title=f"Download {self.spectral_index} data of {shape} for the period {start_date} to {end_date}",
    ...
    def _download_datacube(self, shapefile_name, start_date, end_date):
        ...
        filename = os.path.join(output_dir, f"s3_slstr_{self.spectral_index}_{shapefile_name}_{start_date}_{end_date}.nc")
        ...
        self._download_workflow_lst(connection, filename, shape, start_date, end_date)
        logger.info(f"Downloaded {self.spectral_index} datacube to {filename}")

        # Read the datacube
        ds = xr.open_dataset(filename)
        ds = ds.rename_vars({"var": self.spectral_index})
        ds = ds.where((ds[self.spectral_index] >= -50) & (ds[self.spectral_index] <= 60))
        ds = ds.where(np.isfinite(ds[self.spectral_index]))
        ds = self._filter_by_valid_pixels(ds)
        return ds
    ...
    def _read_datacube(self, start_date, end_date, time_indices=None):
        ...
        if "var" in ds_full:
            ds_full = ds_full.rename_vars({"var": self.spectral_index})
        elif self.spectral_index not in ds_full:
            raise ValueError(
                f"Expected '{self.spectral_index}' or 'var' variable in datacube, "
                f"found: {list(ds_full.data_vars)}"
            )
        ...
        lst = datacube_subset[self.spectral_index]
        lst = lst.where(np.isfinite(lst))
        ds = lst.to_dataset(name=self.spectral_index)
    ...
    def get_debug_image(self):
        ...
        lst_debug = self.datacube[self.spectral_index].isel(t=time_index).values

        # Configure the color palette
        norm = colors.Normalize(vmin=-20, vmax=50)
        rgba = cm.get_cmap("inferno")(norm(lst_debug))
        rgb = (rgba[:, :, :3] * 255).astype(np.uint8)
        lst_debug_img = Image.fromarray(rgb, mode="RGB")
        logger.info("Debug image generated successfully")

        return lst_debug_img
