...
class BSI(L3_Algorithm):
    ...
    def process_data(self, input, l2_datacube: Optional[xr.Dataset] = None) -> List[L3_result]:
        ...
        bsi_debug = bsi.isel(t=0).values
        # Configure the color palette
        norm = colors.Normalize(vmin=-1, vmax=1)
        rgba = cm.get_cmap("YlOrBr")(norm(bsi_debug))
        rgb = (rgba[:, :, :3] * 255).astype(np.uint8)
        bsi_debug_img = Image.fromarray(rgb, mode="RGB")
        result = [L3_result(debug_image=bsi_debug_img, algorithm_results=bsi, time_indices=list(self.time_indices), result_type="datacube")]
        return result
