...
class TestL3LulcClassification(unittest.TestCase):
    ...
    def _find_lulc_result(self, results):
        for result in results:
            ar = result.algorithm_results
            if isinstance(ar, xr.DataArray) and ar.attrs.get("result_type") == "mask":
                return result
        return None
    ...
    def test_lulc_result_type_is_mask(self):
        ...
        lulc_result = self._find_lulc_result(l3_results)
        self.assertIsNotNone(lulc_result, "Should have a result with result_type='mask'")
        self.assertEqual(lulc_result.result_type, "datacube")
    ...
    def test_lulc_mask_has_valid_dimensions(self):
        ...
        mask = lulc_result.algorithm_results.values
